...
// .wrangler/tmp/bundle-4qH8Vw/checked-fetch.js
...
app.use("*", logger());
app.use("*", cors());
async function logError(db, action, error, context) {
  try {
    ...
    await db.prepare("INSERT INTO error_logs (id, action, message, stack, context, timestamp) VALUES (?, ?, ?, ?, ?, ?)").bind(entry.id, entry.action, entry.message, entry.stack, entry.context, entry.timestamp).run();
...
async function logAudit(db, action, type, target, id) {
  try {
    const auditId = `audit_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    await db.prepare("INSERT INTO audit_logs (id, action, type, target, target_id, timestamp) VALUES (?, ?, ?, ?, ?, ?)").bind(auditId, action, type, target, id, (/* @__PURE__ */ new Date()).toISOString()).run();
  } catch (error) {
...
app.post("/lsts", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO lsts (id, title, description, recommendation, severity, status, category, location, identified_date, last_seen_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, lst.title, lst.description, lst.recommendation, lst.severity, lst.status, lst.category, lst.location, lst.identifiedDate, lst.lastSeenDate).run();
...
app.post("/lsts/merge", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO lsts (id, title, description, recommendation, severity, status, category, location, identified_date, last_seen_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(newId, mergedLST.title, mergedLST.description, mergedLST.recommendation, mergedLST.severity, mergedLST.status, mergedLST.category, mergedLST.location, mergedLST.identifiedDate, mergedLST.lastSeenDate).run();
    for (const id of ids) {
...
app.post("/generate-report", rateLimit, async (c) => {
  try {
    ...
    const modelPreference = modelRes[0] ? JSON.parse(modelRes[0].value) : "gemini-flash-latest";
...
app.post("/reports/upload", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO reports (id, title, content, type, metadata) VALUES (?, ?, ?, ?, ?)").bind(id, reportData.title, reportData.content, reportData.type, JSON.stringify(reportData.metadata || {})).run();
...
var PROMPT_TEMPLATE = `Role: You are an expert Medical Simulation Specialist and Education Consultant for the Washington University Department of Emergency Medicine. Your goal is to generate professional, actionable Post-Session Reports that prioritize psychological safety and a "Just Culture" framework.
...
app.get("/model-preference", async (c) => {
  try {
    ...
    return c.json({ model: results[0] ? JSON.parse(results[0].value) : "gemini-flash-latest" });
  } catch (error) {
...
app.post("/model-preference", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value").bind("ai_model_preference", JSON.stringify(model)).run();
...
app.post("/notes/add", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO session_notes (id, session_name, notes, participants, tags, metadata) VALUES (?, ?, ?, ?, ?, ?)").bind(id, note.sessionName, note.notes, JSON.stringify(note.participants || []), JSON.stringify(note.tags || []), JSON.stringify(note.metadata || {})).run();
...
app.get("/case-files", async (c) => {
  try {
    ...
    return c.json(results[0] ? JSON.parse(results[0].value) : []);
  } catch (error) {
    return c.json([]);
  }
});
...
// .wrangler/tmp/bundle-4qH8Vw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4qH8Vw/middleware-loader.entry.ts
